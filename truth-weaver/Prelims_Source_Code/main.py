#!/usr/bin/env python3
"""
Main pipeline: Audio → Transcript → Analysis → JSON Report
Truth Weaver Hackathon Submission
"""
...
TRANSCRIBED_FILE = OUTPUT_DIR / "transcribed.txt"
REPORT_FILE = OUTPUT_DIR / "PrelimsSubmission.json"
...
print("[+] Loading HuggingFace emotion recognition model...")
...
def detect_emotion(segment_file, segment_audio):
    try:
        ...
        return map_emotion(top_label, energy)
    except Exception:
        return ""
...
def run_transcription():
    ...
    for file in sorted(os.listdir(INPUT_DIR)):
        if file.endswith(".wav") or file.endswith(".mp3"):
            ...
            transcripts.append(transcribe_audio(file_path, audio_name))

    with open(TRANSCRIBED_FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(transcripts))

    print(f"[+] Final transcript saved: {TRANSCRIBED_FILE}")
    return TRANSCRIBED_FILE

# ======================================================================
# STEP 2: ANALYSIS (imported from analyze.py, unchanged)
# ======================================================================
# Configure logging
...
class TranscriptProcessor:
    ...
    PROGRAMMING_LANGUAGES = {
        ...
        "java": ["java", "jvm"],
        "javascript": ["javascript", "js", "node", "nodejs", "react", "vue", "angular", "svelte", "nextjs"],
        "typescript": ["typescript", "ts"],
        "c++": ["c++", "cpp", "c plus plus", "cplus"],
        "c#": ["c#", "csharp", "c sharp"],
        "c": ["c language", "ansi c", "iso c"],
        "go": ["golang", "go lang"],
        "rust": ["rust"],
        "ruby": ["ruby", "rails", "ror"],
        "php": ["php"],
        ...
        # Systems / Embedded
        "assembly": ["assembly", "asm", "x86", "arm assembly", "mips assembly"],
        "fortran": ["fortran"],
        "cobol": ["cobol"],
        "ada": ["ada"],
        "vhdl": ["vhdl"],
        ...
        # .NET Family
        "vb.net": ["vb.net", "visual basic .net"],
        ...
        "cuda": ["cuda"],
        "opencl": ["opencl"],
        "fortran modern": ["fortran90", "fortran95"],
        ...
        # Workflow / Infra DSLs
        "terraform": ["terraform", "hcl", "hashicorp language"],
        "ansible": ["ansible", "ansible yaml"],
        "puppet": ["puppet"],
        "chef": ["chef"],
        ...
        # Special mentions
        "celery": ["celery"],   # included as per your request (though it’s a framework, not a lang)
...
class ExperienceExtractor:
    ...
    def extract_experience_claims(self, text: str, session_id: int) -> List[ExperienceClaim]:
        ...
        # --- Pattern 3: Spelled-out numbers (one year, two years, etc.) ---
        ...
        # --- Pattern 5: Long-span words like "decade", "century" ---
        special_timewords = {
            "decade": 10.0,
            "half a decade": 5.0,
            "quarter century": 25.0,
            "century": 100.0,
            "half a century": 50.0,
            "lifetime": 40.0,   # heuristic
            "career": 20.0,     # heuristic average
        }
        for word, years in special_timewords.items():
            if word in text_lower:
                match = re.search(re.escape(word), text_lower)
                context = self._extract_context(text, match.start(), match.end())
                claims.append(ExperienceClaim(
                    raw_text=word,
                    numeric_value=years,
        ...
        # --- Pattern 6: Vague expressions (no numeric value) ---
        vague_patterns = [
            "couple of years", "few years", "several years", "many years",
            "long time", "ages", "extensive experience", "years of experience",
            "plenty of experience", "significant experience", "quite some time",
    ...
    def extract_languages_and_skills(self, text: str) -> Tuple[List[str], List[str]]:
        ...
        for lang, variations in self.processor.PROGRAMMING_LANGUAGES.items():
            for variation in variations:
                patterns = [
                    rf'\b(?:experience|work|code|program|develop|use|using|built|implemented|written|created)\s+(?:in\s+|with\s+)?{re.escape(variation)}\b',
                    rf'\b{re.escape(variation)}\s+(?:programming|development|coding|experience|project|developer)\b',
        ...
        EXPANDED_TECHNICAL_SKILLS = {
            # --- Frontend Frameworks & UI ---
            "frontend": [
                "frontend", "front-end", "html", "css", "javascript", "typescript",
                "react", "reactjs", "react.js", "redux", "nextjs", "next.js",
            ...
            "backend": [
                "backend", "back-end", "node", "nodejs", "express", "koa", "nestjs",
                "spring", "spring boot", "hibernate", "struts",
                "django", "flask", "fastapi", "tornado", "bottle",
                "rails", "ruby on rails",
                "laravel", "symfony", "cakephp", "codeigniter",
                "dotnet", ".net", "asp.net", "wcf", "blazor"
            ],

            # --- Databases ---
            "databases": [
                "mysql", "postgres", "postgresql", "mariadb", "oracle", "db2",
                "sqlite", "mssql", "sql server",
                "mongodb", "couchdb", "couchbase", "dynamodb", "cosmosdb",
                "neo4j", "arangodb", "janusgraph",
                "elasticsearch", "solr", "opensearch", "redis", "memcached",
            ...
            "cloud": [
                ...
                "firebase", "cloudflare", "digitalocean", "linode", "heroku", "vercel"
            ],

            # --- DevOps & Infrastructure ---
            "devops": [
                "docker", "kubernetes", "k8s", "containerd", "podman", "lxc",
                "terraform", "ansible", "chef", "puppet", "packer",
                "jenkins", "gitlab ci", "github actions", "circleci", "travis", "bamboo",
                "argo", "argo cd", "helm", "spinnaker", "vault", "consul",
                "nginx", "apache", "haproxy", "istio", "envoy", "linkerd",
                "prometheus", "grafana", "elk", "elasticsearch logstash kibana",
                "datadog", "newrelic", "splunk", "opentelemetry", "jaeger", "zipkin"
            ],

            # --- Big Data & Streaming ---
            "big_data": [
                "hadoop", "spark", "pyspark", "hive", "pig", "mapreduce",
                "kafka", "flink", "storm", "pulsar", "kinesis", "samza"
            ],

            # --- Data Engineering & Warehousing ---
            "data_engineering": [
                "airflow", "luigi", "dbt", "snowflake", "redshift", "bigquery", "databricks"
            ],

            # --- Data Science & ML ---
            "data_science": [
                "numpy", "pandas", "scipy", "matplotlib", "seaborn", "jupyter", "notebook",
                "scikit-learn", "ml", "machine learning", "ai", "artificial intelligence",
                "tensorflow", "keras", "pytorch", "mxnet", "theano", "caffe",
                "huggingface", "transformers", "onnx", "spacy", "nltk",
                "openai", "langchain", "llamaindex"
            ],

            # --- Cybersecurity ---
            "cybersecurity": [
                "firewall", "ids", "ips", "penetration testing", "pentest",
                "owasp", "nmap", "metasploit", "burp suite", "wireshark",
                "kali linux", "snort", "suricata", "ossec"
            ],

            # --- Testing & QA ---
            "testing": [
                "selenium", "junit", "pytest", "unittest", "mocha", "chai", "cypress",
                "jasmine", "karma", "protractor"
            ],

            # --- Mobile Development ---
            "mobile": [
                "android", "kotlin", "java android", "ios", "swift", "swiftui",
                "react native", "flutter", "xamarin", "cordova", "ionic", "kivy"
            ],

            # --- Operating Systems & Tools ---
            "systems": [
                "linux", "unix", "windows server", "macos", "bash", "zsh", "powershell",
                "systemd", "kvm", "qemu", "vagrant", "docker compose"
            ],
            ...
            "productivity": [
                "jira", "confluence", "trello", "asana", "slack", "teams", "notion", "clickup"
            ],

            "architecture": [
                "architecture","architect","competency framework", "expertise structure", "ability model", "skillset organization"
            ]
        ...
        for skill_category, keywords in EXPANDED_TECHNICAL_SKILLS.items():
...
class TruthAnalyzer:
    ...
    def synthesize_results(self, analyses: List[SessionAnalysis], sessions: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Pick most common language if available
        programming_language = None
        if language_counter:
            programming_language, _ = language_counter.most_common(1)[0]
...
def main():
    ...
    transcribed_path = run_transcription()

    # Step 2: Load transcripts
    sessions = TranscriptProcessor.load_sessions(folder=transcribed_path.parent)
    ...
    # Step 4: Save JSON
    ReportGenerator.save_report(report, REPORT_FILE)
    ReportGenerator.print_summary(report)

    print(f"\n[+] Submission ready: {REPORT_FILE}")
